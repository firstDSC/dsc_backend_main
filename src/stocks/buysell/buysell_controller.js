...
export const addBuy = async (buy_info) => { 
    ...
    await redis.buyEnqueue(key, -score, value);
}
...
export const addSell = async (sell_info) => {
    ...
    await redis.sellEnqueue(key, score, value);
}
...
export const purchaseBuy = async (stockId) => { 
    ...
    const n = await redis.buyCount(stockId);
    
    if(n>0){ 
        const outstanding = await redis.getBuyAll(stockId);
        for(var i=0;i<n;i++){
            const current_price = await redis.getPrice(outstanding[i]);
...
export const purchaseSell = async (stockId) => { // 실시간 시세
    ...
    const n = await redis.sellCount(stockId);
    if(n>0){
        const outstanding = await redis.getSellAll(stockId);
        for(var i=0;i<n;i++){
            const current_price = await redis.getPrice(outstanding[i]);
